            ...
            <div class="card card-stats mb-4 mb-xl-0">
                <div class="card-body">
                    <div class="row">
                        ...
                        <span class="h2 font-weight-bold mb-0 text-dark" style="font-size: 25px;" id="total_sales">{{ number_format($product_profits[0]->total + $accessory_profits[0]->total) }} ks</span>
                    </div>
                    <div class="col-auto">
            ...
            <div class="card card-stats mb-4 mb-xl-0">
                <div class="card-body">
                    <div class="row">
                        ...
                        <span class="h2 font-weight-bold mb-0 text-dark" style="font-size: 25px;" id="total_revenue">{{ number_format($ph_sales_revenues + $acc_sales_revenues) }} ks</span>
                    </div>
                    <div class="col-auto">
                        <div class="icon icon-shape text-white rounded-circle shadow" style="background-color:#473C70;">
                            <i class="fas fa-balance-scale"></i>
      ...
      if(isChanged())
        $.ajax({
           ...
           success:function(data){
               console.log(data);
               var html = '';
             	$('#total_sales').text(Math.round(data.total_sales).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
             	$('#product_profit').text(Math.round(data.product_profits).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
             	$('#accessory_profit').text(Math.round(data.accessory_profits).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
             	$('#ph_sale_revenue').text(Math.round(data.ph_sales_revenues).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
             	$('#acc_sale_revenue').text(Math.round(data.acc_sales_revenues).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
             	$('#total_revenue').text(Math.round(data.total_revenues).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+' ks');
